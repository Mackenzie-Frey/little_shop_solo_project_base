...
RSpec.describe 'Merchant Items' do
  context 'as a merchant' do
    ...
    describe 'when I visit /dashboard' do
      ...
      it 'should show a link to download csv for previous buyers' do
        allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(@merchant)

        visit dashboard_path
        click_on 'Previous Buyers'
        expect(current_path).to eq('/dashboard.csv')
      end
      it 'should show a link to download csv for non buyers' do
        allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(@merchant)

        visit dashboard_path
        click_on 'Non-buying Users'
        expect(current_path).to eq('/dashboard.csv')    
      end
    end
